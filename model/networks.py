...
import numpy as np
from libr.nn.modules import SynchronizedBatchNorm2d as SynBN2d
...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, gpu_ids=None, skip=False, opt=None):
    if gpu_ids is None:
        gpu_ids = []
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc,
        ...
        if self.opt and self.opt.use_norm == 0:
            if outermost:
